...
def cam_to_w(pts, T_w_imu, I2V):
    T_cam_to_velo = np.array([
        [0, 0, 1],
        [-1, 0, 0],
        [0, -1, 0]])
    pts = pts @ T_cam_to_velo.T # to velo
    pts = np.hstack([pts, np.ones((len(pts), 1))])
    pts = pts @ np.linalg.inv(I2V).T   # velo to imu
    pts = pts @ T_w_imu.T    # imu to world coordinate
    pts /= pts[:,-1].reshape(-1,1)
    pts = pts[:,:3]
    return pts

def w_to_cam(pts, T_w_imu, I2V):
    pts = np.hstack([pts, np.ones((len(pts), 1))])
    pts = pts @ np.linalg.inv(T_w_imu).T # to imu coordinate
    pts = pts @ I2V.T # to velo
